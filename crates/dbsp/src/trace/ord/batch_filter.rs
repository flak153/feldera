    ...
    K: DataTrait + ?Sized,
{
    ...
    pub(crate) fn from_file(
        ...
        let membership_filter = match membership_filter {
            Some(BatchKeyFilter::Bloom(filter)) => {
                Some(Arc::new(filter) as Arc<dyn BatchFilter<K>>)
            }
            Some(BatchKeyFilter::RoaringU32(filter)) => {
                Some(Arc::new(filter) as Arc<dyn BatchFilter<K>>)
            }
            None => None,
        };
